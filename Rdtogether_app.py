...
ollama_url = "https://35ea32118292.ngrok-free.app/v1/chat/completions"
model = "gemma3:12b"
...
def ข่าวล่าสุด():
    ...
    for ชื่อหมวด, urls in หมวดข่าว.items():
        หัวข้อในหมวด = []
        for url in urls:
            feed = feedparser.parse(url)
            for entry in feed.entries[:2]:  # ดึงแค่ 2 ข่าวล่าสุดต่อ URL
                หัวข้อ = entry.title.strip()
                ลิงก์ = entry.link
                หัวข้อในหมวด.append(f"- {หัวข้อ} ([อ่านเพิ่ม]({ลิงก์}))")
        if หัวข้อในหมวด:
            สรุปข่าว += f"🔸 **{ชื่อหมวด}**\n" + "\n".join(หัวข้อในหมวด) + "\n\n"

    return สรุปข่าว.strip() if สรุปข่าว else "❌ ไม่มีข่าวใหม่ที่ดึงได้ในขณะนี้ค่ะ"
